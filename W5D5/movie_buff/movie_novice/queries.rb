...
def pulp_fiction_actors
  ...
  Actor.joins(castings: :movie).where(movies: {title: 'Pulp Fiction'}).select('actors.id, actors.name')
end
...
def uma_movies
  ...
  # hint: use 'select', 'joins', 'where', and 'order'
    Actor.joins(castings: :movie).where(actors: {name: 'Uma Thurman'}).select('movies.id, movies.title, movies.yr ')
end
